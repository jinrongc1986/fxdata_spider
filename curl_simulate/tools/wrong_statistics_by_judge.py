...
from curl_simulate.tools.mysql_db import get_uri_by_md5
...
def statics_service_log(timestamp):
    ...
    file_exist = os.path.exists(filepath)
    if file_exist is True:
        f = open(filepath)  # 打开错误的日志
        x = open(filepath + "-statistics", 'w')  # 打开统计日志
        info = f.read().split('***************************************')
        account = len(info) - 1
